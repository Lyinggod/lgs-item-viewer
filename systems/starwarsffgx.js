 ...
 */
 ...
 export const fullSystemName = "Star Wars FFG Standard Rules";
...
export function formatPrice(item, price) {
  const priceRange = getPriceRange(item.system.description || item.system.biography);
  const symbol = game.settings.get('lgs-item-viewer', 'currencySymbol');
  const symbolLocation = game.settings.get('lgs-item-viewer', 'currencySymbolLocation');

  function applyCurrency(value) {
    const formatted = Number(value).toLocaleString();
    return symbolLocation === "$#" ? `${symbol}${formatted}` : `${formatted}${symbol}`;
  }
  ...
  if (priceRange !== "") {
    ...
    const lowFormatted = applyCurrency(low);
    const highFormatted = applyCurrency(high);
    formattedPrice = `${lowFormatted}-${highFormatted}`;
  } else {
    if (price !== undefined && price !== null) {
      const numericPrice = typeof price === 'string' ? parseFloat(price) : price;
      if (!isNaN(numericPrice)) {
        formattedPrice = applyCurrency(numericPrice);
      } else {
  ...
  let restrictedPath = "";
  if (item.system.rarity?.isrestricted) restrictedPath = 'system.rarity.isrestricted';
  if (item.system.stats?.rarity?.isrestricted) restrictedPath = 'system.stats.rarity.isrestricted';
  const isRestricted = getNestedProperty(item, restrictedPath);
